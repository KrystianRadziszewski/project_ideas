...
const Form = ({ type, post, setPost, submitting, handleSubmit }) => {
	return (
		<section className=" w-full max-w-full flex_start flex-col">
			...
			<p className="desc text-left max-w-md">{type} Lorem ipsum dolor sit amet.</p>
			...
			<form onSubmit={handleSubmit} className="mt-10 w-full max-w-2xl flex flex-col gap-7 glassmorphism">
				...
				<label>
					<span className="font-satoshi font-semibold text-base text-gray-700">
						#Tag {` `}
						<span className="font-normal">(product, idea, etc.)</span>
					</span>

					<input type="text" value={post.tag} onChange={(e) => setPost({ ...post, tag: e.target.value })} placeholder="tag" required className="form_input" />
				</label>
